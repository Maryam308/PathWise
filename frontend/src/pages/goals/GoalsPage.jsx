...
const GoalsPage = () => {
  // ── All goals data + mutations come from context ──────────────────────────
  const {
    goals, expenses,
  ...
  // ── Local UI-only state (nothing shared, so stays local) ──────────────────
  ...
  const handleDelete = async (goalId) => {
    try {
      ...
      // Error is surfaced via context's error field — or you can set local error here
  ...
  // ── After projection saves ────────────────────────────────────────────────
  // Re-fetch so the updated monthlySavingsTarget and status are reflected everywhere
  ...
  // ── maxAllocatable for GoalModal ──────────────────────────────────────────
  // Excludes the editing goal's own target so it isn't double-counted
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 pt-[64px]">
        <GoalsHeader goals={goals} onNewGoal={openCreateModal} onViewProjections={() => {}} />
        ...
        <div className="max-w-6xl mx-auto px-6 py-10">
          ...
          {error && (
            <div className="mb-6 p-4 bg-red-50 border border-red-100 rounded-xl flex items-center gap-3">
              ...
              <button onClick={refreshGoals} className="ml-auto text-xs text-red-500 underline font-semibold">Retry</button>
            </div>
          )}
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => handleDelete(deleteConfirm)}
                disabled={mutating}
                className="flex-1 py-2.5 bg-red-500 hover:bg-red-600 disabled:bg-gray-200 text-white font-bold rounded-xl text-sm transition-colors">
      ...
      {/* AICoachWidget reads from context itself — no props needed for goals */}
      <AICoachWidget />
    </div>
  );
};
