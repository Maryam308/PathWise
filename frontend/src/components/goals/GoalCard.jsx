...
import { STATUS_CONFIG, PRIORITY_BADGE, CATEGORY_ICON } from "../../constants/goals.js";
import { formatBD, formatDeadline } from "../../utils/formatters.js";
...
const ContextMenu = ({ goal, onEdit, onProjection, onSimulation, onDelete, onClose }) => (
  <div className="absolute right-0 top-8 z-20 w-44 bg-white rounded-xl shadow-lg border border-gray-100 py-1">
    <MenuButton onClick={() => { onEdit(goal);       onClose(); }} icon={<EditIcon />}>Edit goal</MenuButton>
    <MenuButton onClick={() => { onProjection(goal); onClose(); }} icon={<ProjectionIcon />}>Projection</MenuButton>
    <MenuButton onClick={() => { onSimulation(goal); onClose(); }} icon={<SimulationIcon />}>Simulation</MenuButton>
    <div className="h-px bg-gray-100 my-1" />
    <MenuButton onClick={() => { onDelete(goal.id);  onClose(); }} icon={<TrashIcon />} danger>Delete</MenuButton>
  </div>
);

const MenuButton = ({ onClick, icon, children, danger = false }) => (
...
// SVG icon atoms — keeps JSX readable above
const iconCn = "shrink-0";
const CalendarIcon = () => (
...
const EditIcon = () => (
  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={iconCn}>
